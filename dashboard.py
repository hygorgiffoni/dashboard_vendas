'''Script para gerar um app de dashboard no streamlit'''
...
def format_number(valor, prefixo = ''):
    'Função para formatar o número em milhões, milhares ou unidade'
    for unidade in ['', 'mil']:
        if valor <1000:
...
response = requests.get(URL, timeout=10, params=query_string)
...
.groupby(pd.Grouper(freq = 'M'))['Preço'].count().reset_index())
...
with aba3:
    ...
    with col1:
        ...
        fig_receita_vendedores = px.bar(
            vendedores[['sum']]
            .sort_values('sum', ascending=False).head(qtd_vendedores),
            x = 'sum',
            y = vendedores[['sum']]
            .sort_values('sum', ascending=False).head(qtd_vendedores).index,
            text_auto=True,
    ...
    with col2:
        ...
        fig_vendas_vendedores = px.bar(
            vendedores[['count']]
            .sort_values('count', ascending=False).head(qtd_vendedores),
            x = 'count',
            y = vendedores[['count']]
            .sort_values('count', ascending=False).head(qtd_vendedores).index,
            text_auto=True,
